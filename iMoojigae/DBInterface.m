...
-(NSString *)dbPath
{
    if(!_dbPath)
    {
        ...
        NSString *databaseFilename =@"moojigae.sqlite";
...
-(int)createTable:(NSString *)filePath
{
    ...
    {
        char * query ="CREATE TABLE IF NOT EXISTS article ( boardId TEXT, boardNo TEXT, cr_date DATE DEFAULT (datetime('now','localtime')), PRIMARY KEY (boardId, boardNo))";
        char * errMsg;
...
-(int)searchWithBoardId:(NSString *)boardId BoardNo:(NSString *)boardNo
{
    ...
    if ( sqlite3_open(dbfile, &db) == SQLITE_OK) {
        const char *sql = [[NSString stringWithFormat:@"SELECT count(*) FROM article WHERE boardId = \"%@\" AND boardNo = \"%@\";", boardId, boardNo] UTF8String];
        sqlite3_stmt *stmt;
...
-(void)insertWithBoardId:(NSString *)boardId BoardNo:(NSString *)boardNo
{
    ...
    if(sqlite3_open([self.dbPath UTF8String], &db) == SQLITE_OK) {
        const char *sql = "INSERT INTO article (boardId, boardNo) VALUES (?, ?)";
        sqlite3_stmt *stmt;
        if( sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK) {
            sqlite3_bind_text(stmt, 1, [boardId UTF8String], -1, NULL);
            sqlite3_bind_text(stmt, 2, [boardNo UTF8String], -1, NULL);
